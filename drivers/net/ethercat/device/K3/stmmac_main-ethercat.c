				   ...
				   struct dma_desc *np, struct sk_buff *skb)
{
	...
	if (priv->plat->core_type == DWMAC_CORE_GMAC4 || priv->plat->core_type == DWMAC_CORE_XGMAC)
		desc = np;
...
static int stmmac_init_tstamp_counter(struct stmmac_priv *priv,
				      u32 systime_flags)
{
	bool xmac = (priv->plat->core_type == DWMAC_CORE_GMAC4) || (priv->plat->core_type == DWMAC_CORE_XGMAC);
...
static void stmmac_common_interrupt(struct stmmac_priv *priv)
{
	...
	xmac = (priv->plat->core_type == DWMAC_CORE_GMAC4) || (priv->plat->core_type == DWMAC_CORE_XGMAC);
...
static int stmmac_hw_init(struct stmmac_priv *priv)
{
	...
	if (((priv->synopsys_id >= DWMAC_CORE_3_50) ||
	    (priv->plat->core_type == DWMAC_CORE_XGMAC)) && (!priv->plat->riwt_off)) {
		priv->use_riwt = 1;
